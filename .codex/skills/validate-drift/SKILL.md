...
name: validate-drift
description: >
  Detect drift between Zod schemas in `src/lib/validation.ts` (canonical) and inline validation in server actions or route handlers. Reports fields missing on one side, range mismatches, and enum-value disagreement. Zod is canonical (handbook §3.3); skill's verdict reads "action diverges from schema". Run when editing any server action or schema.
---
