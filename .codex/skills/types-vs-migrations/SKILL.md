...
rg --no-heading "alter table (public\.)?$T (add|drop|rename) column" supabase/migrations/

# Renames (table-level)
rg --no-heading "alter table (public\.)?[a-z_]+ rename to (public\.)?$T" supabase/migrations/
```
