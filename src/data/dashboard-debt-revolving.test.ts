...
import type { BudgetLineRow, LiabilityRow, ProfileRow } from "@/data/supabase/types";

const h = vi.hoisted(() => {
  const profile: ProfileRow = {
    id: "revolving-debt-client",
    profile_type: "client",
    advisor_user_id: null,
    display_name: "Revolving Debt Client",
    monthly_income: "8000",
  ...
  const budgetLine: BudgetLineRow = {
    id: "budget-revolving",
    user_id: "revolving-debt-client",
    category: "debt repayments - credit card",
    cadence: "monthly",
    amount: "200",
    calendar_year: null,
    start_year_month: "2026-06",
    end_year_month: null,
    source_liability_id: "revolving-1",
    created_at: "2026-01-01T00:00:00Z",
  };
...
describe("getDashboardPayload — revolving debt from budget sync", () => {
  it("schedules repayments from linked budget lines when liability repayment is unset", async () => {
    const payload = await getDashboardPayload(
      supabase,
      "revolving-debt-client",
      "2026-06"
    );
    ...
    expect(firstPoint?.requiredDebtRepayment).toBe(2_400);

    const laterPoint = points[1];
    expect(laterPoint?.projectedLiabilities ?? laterPoint?.liabilities ?? 5000).toBeLessThan(
      5_000
    );
  });
});
