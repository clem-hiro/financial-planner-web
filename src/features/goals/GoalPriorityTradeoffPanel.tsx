...
export async function GoalPriorityTradeoffPanel({
  ...
  return (
    <section className="rounded-xl border border-zinc-200/90 bg-zinc-50/50 px-4 py-4 sm:px-5">
      ...
      {!hasIncome ? (
        ...
        <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-zinc-500">Take-home (month)</dt>
            <dd className="font-medium tabular-nums text-zinc-900">
              {formatCurrency(takeHomeMonthly, currency)}
            </dd>
          </div>
          <div>
            <dt className="text-zinc-500">Spend basis (month)</dt>
            <dd className="font-medium tabular-nums text-zinc-900">
          ...
          <div>
            <dt className="text-zinc-500">Surplus before goals</dt>
            <dd className="font-medium tabular-nums text-zinc-900">
          ...
          <div>
            <dt className="text-zinc-500">Total planned to goals</dt>
            <dd className="font-medium tabular-nums text-zinc-900">
      ...
      {lines.length > 0 ? (
        <ul className="mt-4 space-y-2">
          {lines.map((line) => (
            <li
              key={line.goalId}
              className="flex flex-wrap items-baseline justify-between gap-2 rounded-lg border border-zinc-100 bg-white px-3 py-2 text-sm"
            >
