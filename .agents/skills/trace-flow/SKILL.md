---
name: trace-flow
description: Trace the forward call chain from an entry point through the layered architecture (UI → server action → repository → supabase). Reads docs/function-tree/function-tree.json. Use to understand which layers a request touches, or to verify a layer boundary is respected.
allowed-tools: Read, Bash, Grep
argument-hint: "<function-id-or-symbol>  [--max-depth N]"
---
...
Same logic as `/blast-radius` Phase 2 — resolve `$ARGUMENTS` to a fully-qualified
