...
| Housing quick-add / guided property + BSD; CPF-aware instalment funding | **Shipped** | `payment_source` (`cash` \| `cpf_oa` \| `split`) + optional `cpf_oa_payment` / `cash_payment` on **`financial_housing_loans`**; cash portion flows into dashboard/budget synthetic spend; OA portion stays in CPF projection (`housing-loan-payments.ts`, `HousingPaymentSourceFields`). Legacy rows without `payment_source` default cash for budgeting; CPF still uses `oa_share_of_payment`. |
...
_Last reviewed (2026-05-19): **CPF-aware housing loan instalments** — `payment_source` / split amounts on `financial_housing_loans`; cash vs OA paths separated in dashboard budget, surplus accrual, and CPF monthly projection; Setup/Wealth housing UI updated. Prior: consent-gate Phase 2 (2026-05-18)._
