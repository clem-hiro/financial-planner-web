...
| Budget lines, overrides, strategy insights, recurring budget review, irregular expense reserves | **Shipped** | Repositories + `src/domain/finance/budget*.ts`; budget workspace includes month-by-month review, overrides, unbudgeted spend, scheduled/inactive lines, annual / irregular reserves, and a **Recommended Budget** card (`BudgetRecommendedPlanCard`) that generates a calm monthly allocation from profile assumptions + income and can apply it while preserving debt/tax lines. Aligns with guidance-first IA (Profile = assumptions; Budget = recommendation). |
