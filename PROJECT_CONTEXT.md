...
| `/advisor/client/[id]` | **Advisor** client workspace with **Overview \| Compose \| Proposals** sub-tabs (`?view=overview\|compose\|proposals&p=<id>`): read-only Overview (header, projection toggle + "Compose proposal" button, health, cashflow, read-only goals/budget/investments — `AdvisorClientOverview`); editable Compose (`?view=compose`: suggestion-mode profile/goals/budget/investment edits + draft submit — `AdvisorClientCompose`); proposals history table + per-row **Withdraw** + per-proposal detail (`AdvisorClientDetailShell`). |
