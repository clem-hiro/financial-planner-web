...
| Per-proposal compare projections | **Shipped** | Advisor and client per-proposal detail views render actual-vs-proposed projection compare. _(Code complete; DB migrations `20260601`–`20260605` hand-apply pending.)_ |
...
| `/advisor/client/[id]` | **Advisor** client workspace with **Overview \| Proposals** sub-tabs (`?view=overview\|proposals&p=<id>`): suggestion-mode profile/goals/budget/investment edits + submit, dashboard-month cashflow readouts (`AdvisorClientWorkspace`); proposals history table + per-row **Withdraw** + per-proposal detail (`AdvisorClientDetailShell`). |
