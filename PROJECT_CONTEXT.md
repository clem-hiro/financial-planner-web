...
| Vehicle planning (SG COE/PARF, loans) | **Shipped** | Setup → vehicles; included in Wealth and long-horizon projections. Saved vehicles render as collapsed rows by default, expand to show SG valuation detail, edit fields, and delete action. The add-vehicle pane is also collapsed by default, with nested disclosure groups for identity, market estimate, OneMotoring values, COE expiry recovery, and advanced assumptions. |
