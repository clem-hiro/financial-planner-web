...
| **Advisor cash + liability propose-CRUD** (Phases 2–3) | **Shipped** | Advisor compose can **create / update / delete cash accounts and liabilities** as staged proposals across all 5 seams: compose actions (`src/server/advisor-client-actions.ts`) with Zod allowlist + `entityNameUniquenessError` + the per-category gate; shared overlay branch (`apply-overlay.ts`); accept writer (`apply-changes.ts`, via the generic `applyEntityDiff`) with per-type name-conflict guard; and a shared **locked "Private" card** (`LockedCategoryCard`) in `AdvisorClientCompose` when the client hasn't shared the category. Liabilities lack `updated_at` ⇒ liability change rows carry **no `base_version`** (no optimistic-concurrency guard — whole-row last-write-wins on accept; delete-races safe). Vehicle / property / housing-loan (Phases 4–6) **not yet built** — accept rejects those `entity_type`s. _(Migrations `20260624`/`20260625` on prod, verified 2026-05-24.)_ |
