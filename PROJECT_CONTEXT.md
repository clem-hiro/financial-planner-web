...
| Budget lines, overrides, strategy insights, recurring budget review, irregular expense reserves | **Shipped** | Repositories + `src/domain/finance/budget*.ts`; budget workspace includes month-by-month review, overrides, unbudgeted spend, scheduled/inactive lines, annual / irregular reserves, a **Recommended Budget** card, and a collapsed **Assumptions in play** cue on the Budget hero (take-home / bonus / pay rises → Profile). Aligns with guidance-first IA (Profile = inputs; Budget = recommendation + light assumption review). |
