...
| **Advisor cash-account propose-CRUD** (Phase 2) | **Shipped** | Advisor compose can **create / update / delete cash accounts** as staged proposals across all 5 seams: compose actions (`src/server/advisor-client-actions.ts`) with Zod allowlist + `entityNameUniquenessError` + the per-category gate; shared overlay branch (`apply-overlay.ts`); accept writer (`apply-changes.ts`, via the generic `applyEntityDiff`) with cash name-conflict guard; and a **locked "Private" card** in `AdvisorClientCompose` when the client hasn't shared the category. Reusable pattern for liability / property / housing-loan / vehicle (Phases 3–6, **not yet built** — accept rejects those `entity_type`s). _(Migrations `20260624`/`20260625` on prod, verified 2026-05-24.)_ |
