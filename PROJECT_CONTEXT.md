...
| Capability | Status | Notes |
|------------|--------|--------|
| Net worth, savings, month-scoped metrics | **Shipped** | `getDashboardPayload` (`src/data/dashboard.ts`). Net worth includes investments, cash, optional CPF, vehicles, current property net equity, and liabilities. Home **Overview** uses a compact hero, sticky on-page subnav (Overview / Projected wealth / This month), unified monthly KPI cards with emerald accent on safe-to-spend only, visible net-worth breakdown, and contextual spend-basis action link. |
