...
| **Consent-gated advisor access** + client consent control | **Shipped** | Advisor reads of client financial data require **active client consent**, enforced in-DB via SECURITY DEFINER `advisor_read_*` RPCs (chokepoint; legacy direct-advisor `financial_*` RLS dropped). Client grants/withdraws on **`/more`** → Privacy & Advisor Access (`ClientConsentControl`) + shell `ClientConsentBanner`; consent dialog on **Contact advisor** (`ContactAdvisorButton`). Append-only `advisor_client_consents` ledger, latest-event-wins with monotonic `seq`; per-event verbatim `consent_text`/`consent_version` (`2026-05-18.option-b`). Migration `20260529000000`; ship gate `verify_consent_gated_access()` = `OK` on prod. |
...
| Review advisor-proposed plan changes | **Shipped** | Full-page **`/review/proposal/[id]`** (inbox CTA) **+ persistent client history** at **`/setup?tab=advisor-proposals`** → **`/setup/advisor-proposals/[id]`** (survives inbox dismissal) — before/after by section + per-proposal projection compare, advisor note, accept/reject. Canonical tracker updates only on accept; accepting a proposal whose baseline moved underneath **surfaces a conflict instead of overwriting** (per-entity `base_version` optimistic concurrency, atomic zero-write abort, advisor re-baseline inbox alert). _(DB migrations `20260601`–`20260606` applied on prod, verified 2026-05-23.)_ |
...
| Profile income / CPF / retirement assumptions | **Shipped** | Financial Setup **profile** tab; drives projections and safe-to-spend. CPF rules/assumption review reminders now version the calculation baseline and surface inbox + inline CPF review prompts. |
...
| Per-client workspace (read-only Overview + editable Compose) | **Shipped** | RLS-backed. Split into read-only `AdvisorClientOverview` (header, projection toggle, health, cashflow, read-only goals/budget/investments) and editable `AdvisorClientCompose` (`?view=compose`: profile/goals/budget/investment authoring + draft submit), sharing `AdvisorClientHeader` + `AdvisorConsentRequired`. |
| **Advisor proposal & change review** (suggestion mode) | **Shipped** | Advisors queue **field-level** edits on a draft proposal — **create / update / delete** for investments, budget lines, goals; profile edit-only — and may hold **multiple concurrent pending proposals** (≤1 working draft per client) and **withdraw** any open one. Advisor `[id]` page has **Overview \| Proposals** sub-tabs (`?view=`): authoring lives on the **Compose** view (`?view=compose`, no tab — reached via the read-only Overview's "Compose proposal" button on the projection toggle row); per-proposal detail keeps Proposals active. Client reviews on `/review/proposal/[id]` (inbox) or persistent `/setup/advisor-proposals/[id]`, accepts/rejects before canonical data updates. Inbox CTA via `financial_inbox_notifications` (`kind: advisor_proposal`). _(DB migrations `20260601`–`20260606` applied on prod, verified 2026-05-23.)_ |
...
| **Advisor cash + liability + vehicle + property propose-CRUD** (Phases 2–5) | **Shipped** | Advisor compose can **create / update / delete cash accounts, liabilities, vehicles, and properties** as staged proposals across all 5 seams: compose actions (`src/server/advisor-client-actions.ts`) with Zod allowlist + `entityNameUniquenessError` + the per-category gate; shared overlay branch (`apply-overlay.ts`); accept writer (`apply-changes.ts`, via the generic `applyEntityDiff`; **property uses a bespoke loop** that builds a proposal-scoped `propertyIdMap` draft-key→real-id + `validPropertyIds` set — groundwork for the Phase 6 housing-loan `property_id` remap) with per-type name-conflict guard (name field is `name` for cash/liability/property, **`label`** for vehicle); and a shared **locked "Private" card** (`LockedCategoryCard`) in `AdvisorClientCompose` when the client hasn't shared the category. **Vehicle** exposes the **core 7** fields (full SG valuation set stays in the client's `VehiclesPanel` — advisor create defaults safely, edit preserves). **Property** + cash/investments are **versioned** (`updated_at` ⇒ `base_version` optimistic-concurrency guard); liabilities + vehicles lack `updated_at` ⇒ last-write-wins on accept (delete-races safe). Housing-loan (Phase 6) **not yet built** — accept rejects that `entity_type`. _(Migrations `20260624`/`20260625` on prod, verified 2026-05-24.)_ |
...
| `/advisor/client/[id]` | **Advisor** client workspace with **Overview \| Proposals** sub-tabs (`?view=overview\|compose\|proposals&p=<id>`): read-only Overview (header, projection toggle + "Compose proposal" button, health, cashflow, read-only goals/budget/investments — `AdvisorClientOverview`); editable Compose view (`?view=compose`, no tab — reached via the Overview button: suggestion-mode profile/goals/budget/investment edits + draft submit — `AdvisorClientCompose`); proposals history table + per-row **Withdraw** + per-proposal detail (`AdvisorClientDetailShell`). |
| `/review/proposal/[id]` | **Client** full-page review of an advisor proposal (inbox CTA): section-grouped before/after, advisor message, accept/reject (`ProposalReviewView`). |
| `/setup/advisor-proposals/[id]` | **Client** persistent per-proposal review (independent of inbox dismissal): projection compare + change list, sticky Accept/Reject while pending (`ClientProposalsView`). Reached from `/setup?tab=advisor-proposals`. |
...
- **Per-user unique entity names (`20260623000000_unique_entity_names.sql`):** seven functional unique indexes **`*_user_name_ci_uq`** on **`lower(btrim(<col>))`** per `user_id` — **`financial_investments.name`**, **`financial_goals.title`**, **`financial_liabilities.name`**, **`financial_properties.name`**, **`financial_cash_accounts.name`** (purpose **ignored** — uniqueness is `(user_id, name)`), **`financial_housing_loans.label`**, **`financial_vehicles.label`**. A defensive duplicate-rename (oldest row by `created_at,id` keeps the bare name; later rows get ` 2`, ` 3`, …) runs before each index; **additive-only** — no new tables/columns/RLS. Enforced friendly upstream so the raw `23505` never reaches users: shared validator (`normalizeEntityName`/`entityNameCollides`/`entityNameUniquenessError` in `src/lib/validation.ts`), advisor **compose** guard (`src/server/advisor-client-actions.ts`), **accept-time** `detectNameConflicts` (`src/domain/advisor-proposals/apply-changes.ts`; `ProposalConflict.reason = 'name_in_use'`, rendered in `ProposalReviewView.tsx`), and **client setup CRUD** (`src/server/actions.ts`, 15 create/rename sites). Scratch verification: **`supabase/tests/unique_entity_names_scenarios.sql`** (8-scenario operator runbook). _(**Applied to prod 2026-05-23** via SQL editor; all 7 `*_user_name_ci_uq` indexes verified live (read-only MCP); zero pre-existing duplicates. The app-side friendly guards are **committed but pending deploy** — a collision surfaces a raw `23505` on the live site until the app ships.)_
...
- **Current ownership (shipped):** `financial_properties` + optional linked row on `financial_housing_loans`. User flow: Setup → **Housing** → **Add property** (property details, then “has loan?” → inline mortgage fields). Cash & Debts still holds generic liabilities; housing mortgages are not duplicated there.
...
- **Projections / CPF:** unchanged pipeline — still amortize **`financial_housing_loans`** via `housingLoanToProjection` in `src/data/dashboard.ts`.
...
_Last reviewed (2026-05-24): **Advisor cash propose-CRUD + per-category privacy + proposal-UX batch** — migrations `20260624000000` (per-category `advisor_consent_category_visibility` bridge + default-deny `advisor_can_read_category` gate + 5 sensitive read RPCs redefined) and `20260625000000` (`entity_type` CHECK → 9 types + `advisor_read_category_visibility` flags RPC) **applied to prod + read-back verified 2026-05-24**. Shipped: advisor **cash-account propose-CRUD** (Phase 2 — compose actions + overlay + accept writer + name-conflict guard + locked-card-when-private); **per-category client privacy toggles** (default PRIVATE) on `/more` + setup cash-liabilities/housing/vehicles tabs (switch UI); and the **proposal UX batch** — Compose entry on the Proposals sub-tab, frozen left-column-clipped Submit bar + submit-confirmation dialog, accept confirm + "Proposal Approved" approval dialog (Go to page / Stay), pending-count badge on the client Advisor-Proposals tab + overview card + PENDING setup-hub status, per-proposal review list retitled "Advisor Proposals" grouped by account, centered `ConfirmDialog` for all removal confirms, frozen client Accept bar. Prior (2026-05-23): **Investment Review-due reason tooltip** — each per-account "Review due" badge now shows an `InfoTooltip` stating the specific trigger for that account (stale >12 months with the month count, or no recorded update date). New `investmentReviewReason()` in `src/domain/finance/investment-review.ts` returns the single applicable reason; `investmentRowIsStale` re-derived from it (behavior unchanged for the inbox/count callers). UI-only refinement, no DB/migration change. Prior (2026-05-23): **Per-user unique entity names** — DB-level **case-insensitive, trimmed** name uniqueness per user across the 7 named-entity tables (migration `20260623000000`; 7 `*_user_name_ci_uq` functional indexes on `lower(btrim(<col>))`, additive-only). Friendly upstream guards so no raw `23505` surfaces: advisor **compose** block, **accept-time** friendly `ProposalConflict` (`reason: name_in_use`, raised **pre-claim** so the proposal stays `pending`), and **client setup CRUD** messages — all via the shared validator in `src/lib/validation.ts`. Scratch matrix `supabase/tests/unique_entity_names_scenarios.sql`. Migration **applied to prod 2026-05-23** via SQL editor (all 7 indexes verified live, zero pre-existing duplicates); **app-side guards committed but pending deploy** (raw error on the live site until the app ships). Prior (2026-05-23): **Advisor workspace Overview/Compose split** — the advisor client `[id]` page splits into a read-only `AdvisorClientOverview` and a dedicated editable `AdvisorClientCompose` (`?view=compose`, no tab), reached via a "Compose proposal" button on the projection toggle row (new `ProposalProjectionCompare.action` slot); shared `AdvisorClientHeader` + `AdvisorConsentRequired`. No DB/migration/server-action changes. Prior (2026-05-22): **Goal priority & trade-off views** — `display_order` on `financial_goals`, ↑/↓ reorder, waterfall funding panel vs monthly surplus on Setup/Future goals. Prior same day: debt payoff strategy comparison; cash-flow setup guidance; cash account grouping + balance history. Prior (2026-05-21): **Irregular expense reserves** — budget workspace now translates quarterly, semi-annual, annual, and monthly set-aside inputs into annual plans, with reserve/catch-up guidance for non-monthly costs. Prior (2026-05-21): **Budget review workflow** — recurring monthly review for planned categories, actual spend, temporary overrides, unbudgeted spend, and scheduled/inactive lines. Prior (2026-05-19): **Advisor↔client proposal lifecycle redesign** (migrations `20260601`–`20260606` applied on prod, verified 2026-05-23) — RLS create-unblock RPC, change ops (create/update/delete) + `base_version` optimistic-concurrency, **claim-gated accept** (`pending`→`accepting`→`accepted` SECURITY DEFINER claim/finalize, FOR UPDATE-serialized vs withdraw — zero writes if the claim loses the race; post-claim mid-write failure parks terminally in `accepting`), concurrent multi-pending (draft-only open index), withdraw RPC/action + advisor re-baseline alert, persistent client access `/setup/advisor-proposals/[id]` + advisor Overview\|Proposals sub-tabs, reusable collapsible rail. Prior (2026-05-19): **Asset-first housing** — `financial_properties`, linked mortgages via `property_id`, Housing tab + property-first form, legacy loan backfill. Prior (2026-05-18): **Consent-gate Phase 2 SHIPPED** — advisor→client reads via `advisor_read_*` RPCs; client consent on `/more` + shell banner; migration `20260529000000` on prod with `verify_consent_gated_access() = OK`._
