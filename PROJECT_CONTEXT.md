...
_Last reviewed (2026-05-18): **Consent-gate Phase 2 SHIPPED** — advisor→client data access is now consent-gated via SECURITY DEFINER `advisor_read_*` RPCs (legacy direct-advisor RLS dropped); client consent grant/withdraw on `/more` + shell banner + Contact-advisor dialog; migration `20260529000000` applied to prod with `verify_consent_gated_access() = OK`. Feature inventory is the sole repo roadmap source; BYOFA Notion **Feature Roadmap Table** syncs from inventory (see `.cursor/rules/project-context-notion-sync.mdc`)._
