...
| Investments with contribution phase, step-ups, and withdrawals | **Shipped** | DB migrations `20260516000000_*` + `20260601010000_*` + `20260624000000_*` (`plan_nature`, `contribution_start_date`); Setup/Wealth → Investments; FV helpers support until-retirement / fixed-duration / **calendar premium end** contributions, ILP guidance (pure investment vs bundled cover, double-count reminder), annual contribution step-ups, planned monthly withdrawals, assumption banners, and annual review reminders via `last_investment_review_at` + `investment-review.ts`. |
...
_Last reviewed (2026-05-25): CPF balance-as-of month + Dec year-end CPF snapshot shipped; 2026 CPF allocation ratios, post-55 RA contribution routing, and month-after-birthday age-band handling updated in `cpf-monthly-projection.ts` / `sg-cpf-contribution-buckets.ts`; migration `20260626000000`. Prior: CPF Investments MVP (`20260625000000`), CPFIS-SA maturity helper; `cpf-retirement-projection.ts`._
