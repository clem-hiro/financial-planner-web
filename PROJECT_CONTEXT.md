...
| **Version 2** (current) | **Major UI / IA change:** compact top nav (**Home**, **Setup**, **Activity**, **More**). Setup is the single editor + progress hub (including Protection roadmap and goals coming-modules). Legacy **`/planning/**`** routes redirect into Setup. Home framed as a **command center**. Business logic kept compatible via adapters and redirects. |
...
**Top navigation (client shell)** is intentionally small: **Home**, **Setup**, **Activity**, **More**. Deep or secondary destinations (profile assumptions, methodology, sign out) live in the **account menu** and the **More** hub so the header stays quiet and scalable.

**Home vs Setup vs Activity vs More**

| Surface | Role |
|--------|------|
| **Home** (`/dashboard`) | **Today’s financial command center**: overview metrics (income, expenses, left after expenses, net worth), projected wealth, and this-month budget check — prioritized signals, not every module. |
| **Setup** (`/setup`, `/setup/overview`) | **Single editor + progress hub**: profile, investments, CPF, housing, vehicles, cash & debts, protection roadmap, budget, goals, income tax, advisor proposals. Roadmap placeholder cards live here (hub + Goals / Investments / Protection tabs) — not a second form surface. |
| **Activity** (`/expenses`) | **Cash activity & spending**: expenses, charts, and month-scoped guidance tied to the budget model. |
| **More** (`/more`) | **Secondary destinations**: setup hub, activity — keeps the top bar from becoming a junk drawer. Methodology lives on account menu **How it works** and contextual `(?)` links. |

**Classic / legacy routes** (`/goals`, `/balances`, `/budget`, `/financial-profile`, `/planning/**`) redirect into Setup for continuity and bookmarks.
...
| Client onboarding wizard | **Shipped** | Three-step flow: gross salary + CPF take-home preview → money style + illustrated budget → finish. Lifestyle preferences removed from onboarding (defaults used; editable in Setup → Profile). Seeds `financial_profiles`. See [Onboarding philosophy](#onboarding-philosophy). |
...
| **Client UI v2** shell: Home, Setup, Activity, More | **Shipped** | `AppShell`, `AppShellNav` (`AppShellMobileNav` / `AppShellDesktopNav`); mobile header + scrollable setup tabs; version label `src/lib/client-release.ts`, shown on `/more`. Top-nav label is **Setup** (not Financial setup). Planning removed from top nav — `/planning/**` redirects into Setup. |
...
| Capability | Status | Notes |
|------------|--------|--------|
| Net worth, savings, month-scoped metrics | **Shipped** | `getDashboardPayload` (`src/data/dashboard.ts`). Net worth includes investments, cash, optional CPF, vehicles, current property net equity, and liabilities. Home **Overview** monthly KPIs are **Income** (salary take-home + optional other monthly income), **Expenses** (monthly budget by default; logged expenses when any exist that month), and **Left after expenses** (income − expenses, can be negative; goals not subtracted). Emerald accent on Income; net-worth breakdown and contextual expenses action link retained. |
| Safe to spend / discretionary after goals | **Shipped** | Still computed for Goals & cash flow and other consumers; no longer a Home Overview KPI. Requires income/profile where applicable. |
| Spending vs budget / month health | **Shipped** | Tied to budget lines + expenses. Overview Expenses card uses budget-by-default / logged-when-present. |
| **Illustrative** long-horizon projections (investments, cash surplus, CPF, vehicles, property, combined charts) | **Shipped** | `DashboardRetirementSection`, domain finance modules; methodology links — not advice. Data layer exposes a time-aware retirement cash-flow ledger on each age row (cash-accessible inflows, required outflows, structured outflow components, pre-portfolio gap, component income, investment / ILP principal sold, cash-reserve drawdown, GoalsGap, funded transfers, debt repayment required/funded/unfunded amounts, funded debt principal/interest, and projected liabilities), and the Home retirement section renders the **retirement runway** chart (`RetirementRunwayLedgerChart`) — a warm-light financing-ledger view: each age-year bar totals required outflows but is stacked by how those outflows are financed (take-home, passive income, investment income, investment / ILP drawdown, cash reserve spend-down, shortfall). A top-left toggle switches the chart between cash-off and cash-reserve scenarios; hover and the default-collapsed **Breakdowns** pane expose selected-age Planned expenses, Source, net-worth components including property net equity, and CPF buckets. Breakdowns cards are collapsible and default expanded; the pane matches chart height on wide layouts, stacks on narrow containers, and scrolls internally when details overflow. Milestones render below the chart, and a dark line keeps net worth on a secondary axis. CPF salary/bonus contributions stop at modeled retirement while CPF interest, upfront housing OA events, CPFIS, RA routing, and CPF LIFE continue; recurring mortgage repayments are applied by the unified debt ledger so CPF OA funding and debt reduction are not double-counted. It replaces the removed `AgeCombinedAssetsProjectionChart`. Home no longer shows the **Retirement income (simplified dividend check)** card or the collapsed **Net worth by age** table (redundant with runway + Breakdowns); dividend-vs-spend domain math remains for Setup → Goals assumptions. |
| **CPF retirement projection + CPF Investments** (OA/SA investment entries, FRS/BRS/ERS estimates, age-55 RA simulation, educational scenarios) | **Shipped** | Setup/Wealth → CPF & CPF Investments supports **CPF balance as-of month** (projection starts the following month), multiple OA/SA investment entries with purchase month, single vs regular premium, amount, growth, and maturity month. Home **Projected wealth** shows the CPF-by-age chart (OA/SA/MA/RA) with a one-line year-end total (no standalone year-end card); educational RA-at-55 panel below chart uses pre–set-aside OA/SA from the monthly path. If no CPF balance row exists, forward projections start from virtual $0 CPF buckets while current net worth still only counts saved CPF balances. `buildCpfMonthlyProjectionSeries` uses the 2026 CPF allocation ratios, forms RA at age 55 (SA then OA up to the projected FRS target), credits RA at the SA floor rate, caps MA at the applicable Basic Healthcare Sum (official values through 2026; future years estimated at 4% p.a. until CPF releases figures), overflows MA excess to SA before 55 and to RA from 55, deducts future CPF investment premiums, carries notional CPF investment value, and routes maturity proceeds: OA → OA; SA before 55 → SA; SA after SA closure → RA first, then OA. |
| Embedded “AI insights” as generative product | **Planned** | Roadmap card only; static `InsightCard` / copy where used. |

### Client — Planning (folded into Setup)
...
| Profile income / CPF assumptions | **Shipped** | Setup **profile** tab — income, bonus, optional **other monthly income** (one take-home lump for side hustle/freelance), birth date, CPF salary path. Values seeded from onboarding appear as normal editable fields (no permanent sync banner). **Budget preferences** (life stage, food spend, money style, detail preference) only feed Setup → Budget → Recommended Budget when applied; they do not change income, expenses, or lines until Apply Recommendation. No retirement fields. Home **Income** = salary take-home + other monthly income (rental on Housing stays separate). |
...
| **Setup Hub** (progress, section status, recommended next step) | **Shipped** | `/setup/overview` and bare `/setup` as the stable Setup landing experience; config in `src/domain/setup/modules.ts`, evaluators in `src/domain/setup/evaluators.ts`, loader `src/data/setup-status.ts`. Legacy `/planning/setup` redirects here. |
| Setup tabs (profile → goals) | **Shipped** | `/setup?tab=…` (focused editors); **desktop** grouped sticky side nav (Progress / Core / Wealth / Protection / Future / Advisor); **mobile** horizontal scroll rail. Includes **Protection** (roadmap). Housing and Vehicles precede Cash and debts in the flat tab list for loan-register ordering. Hub links into each tab. |
| Budget lines, overrides, strategy insights, recurring budget review, irregular expense reserves | **Shipped** | Repositories + `src/domain/finance/budget*.ts`; Budget primary spine is Hero → categories → Recommended → unbudgeted (only when needed); secondary tools (needs/wants mix, monthly review, quick add, yearly, advanced, paused lines) sit in collapsed **More on Budget**. No on-page jump rail; Coming soon placeholders removed from strategy insights. Assumptions cue remains on the hero (→ Profile). |)
| SG-oriented guided budget templates (onboarding + domain) | **Shipped** | Modular generator in `budget-guided-setup.ts`; seeded in onboarding and shown on Setup → Budget. Card is client-simple: recommended amounts + Apply when empty; quiet “Replace my budget…” with browser confirm when lines exist. No signal checklist / How-this-was-built panel. |
...
| **Housing (asset-first)** — HDB homes + linked mortgages | **Shipped** | **`financial_properties`** (current ownership) + **`financial_housing_loans.property_id`** (optional mortgage debt). Setup tab **Housing** (`/setup?tab=housing`; legacy `housing-loans` redirects). The Housing tab renders saved property rows first as collapsed rows with linked-mortgage status badges, expanded details, and delete action, then the HDB add flow and manual mortgage entry. **Add HDB home** form is MVP-focused on **BTO** and **Resale HDB**; EC/condo/landed options are visible as planned. It captures purchase price/year, **option fee** (default S$2,000 cash for BTO), **BSD** and **legal fees** as separate editable payment events (CPF vs cash split + paid month each), first/second downpayment, remaining loan amount, **HDB loan default 2.6%** or **bank rate user entry**, and monthly instalment CPF/cash split. CPF projections deduct OA in each event's paid month via `housingUpfrontOaEvents`. Dashboard net worth includes current property net equity. **`planning_scope`**=`future_simulation` reserved for Goals property purchase (no live cashflow). Migrations `20260519120000` + `20260623000000` + `20260628000000`. |
| **Debt planning** (categories, repayment, budget + projection integration) | **Shipped** | `financial_liabilities` extended fields; auto monthly budget lines under "Debt Repayments"; educational UX in Setup / Wealth. Cash and debts is now a consolidated debt register: property and vehicle loan categories are removed from new generic debt entry, source-owned Housing / Vehicle loans render there as collapsed read-only cards with edit links back to their registered setup tab, and duplicate display names get source suffixes. Housing properties without linked mortgage rows render as separate follow-up rows, not debt totals. Loan details capture remaining tenure as **years + months** while storing total months; month overflow rolls into years. Long-horizon projections now use a unified debt ledger for liabilities and housing loans: repayments are funded from cash / CPF OA / split source first, unpaid instalments remain shortfall, and liabilities only reduce by funded principal. Multi-debt **avalanche vs snowball** comparison with optional extra monthly (`debt-payoff-strategies.ts`, `DebtPayoffStrategyComparison`). Amber hint when repayment assumptions are missing. Migration `20260628000000_financial_loans_register.sql`. |
| Methodology / “How it works” | **Shipped** | Account menu + contextual `(?)` / links. Sheet covers trust math only (retirement, free cash flow, CPF, tax, vehicles)—UI how-tos are inline. |
...
| **Per-category client privacy toggles** (default PRIVATE) | **Shipped** | Client controls which of the 5 sensitive categories (cash, liabilities, property, housing loans, vehicles) the advisor may **see + propose on** — switch UI in the **`AdvisorConsentDialog`** on `/more` (when consent active) and inline on the setup **cash-liabilities / housing / vehicles** tabs (shared `CategoryVisibilityToggle`). Master consent stays separate; category visibility is **additive (AND)**, default **PRIVATE** (`advisor_can_read_category` default-deny). |
...
**Card destinations:** Modules that already have Setup editors link to `/setup?tab=…`. Protection / documents roadmap modules also stay under Setup (`?tab=protection` / `?tab=goals`). Financial modules should not point to **More**; More stays account/help/admin.
...
**Debt planning philosophy:** a liability is modeled as both balance-sheet debt (net worth) and a **future cash-flow obligation**. Optional loan metadata drives repayment estimates (amortized, flat-rate, or manual/revolving), syncs **monthly budget lines** under “Debt Repayments”, and feeds the unified retirement debt ledger. Projection balances only fall when principal is actually funded; unpaid instalments stay as shortfall rather than silently improving net worth. Educational copy in the debt UI explains structures without banking jargon.
...
1. **Key metrics** win the viewport first (income, expenses, left after expenses, net worth, month status).
...
| `/more` | Secondary hub: profile, setup, activity, and **Client UI version** strip (`(app)/more/page.tsx`, `src/lib/client-release.ts`). |
...
| `/setup` | Redirects to the **Setup overview** (`/setup/overview`). Focused editors remain at `/setup?tab=profile`, `/setup?tab=housing`, etc. |
...
**Why:** The client shell and **Setup** (`/setup`) were readable on desktop but felt like a compressed dashboard on phones — centered hamburger, heavy top padding, tabs that wrapped or cramped, and module cards pushing primary editors below the fold.
...
**Setup tabs:** `SetupTabsNav` uses a **horizontally scrollable pill rail** below `lg`, and a **grouped sticky side menu** from `lg` up (`SETUP_NAV_GROUPS` in `src/lib/setup-tabs.ts`). The rail/side nav starts with **Overview**, so `/setup/overview` and `/setup?tab=…` read as one setup area. Sticky offset follows the app header. Tab `?tab=` routing and server bundle loading are unchanged.
...
| Onboarding | `src/features/onboarding/` (`OnboardingWizard`, `BonusMonthSelector`, `onboarding-module-sync.ts`), `(app)/onboarding/page.tsx` — gross income + CPF preview → illustrated strategy/budget → finish; `src/domain/finance/onboarding-income.ts`; profile stores gross/derived take-home, `annual_bonus` / `annual_bonus_months`, lifestyle preference fields, optional guided lines via `applyGuidedBudgetLinesAction` |
| Lifestyle Profile (Setup → Profile) | `src/features/setup/BudgetLensProfileForm.tsx` — titled **Budget preferences**; life stage, food spend, money style, detail preference (PATCH `/api/profile`); steers Recommended Budget only; no allocation preview |
| Budget strategy & guided templates | `src/domain/finance/budget-guided-setup.ts` — lifestyle presets, 50/30/20-style splits, modular `BudgetRecommendationContext` / signal metadata, SG-oriented line generator, category→needs/wants/savings heuristics for visuals |
...
| Budget | `src/features/budget/` (`BudgetPlanningView`, `BudgetRecommendedPlanCard` for recommended monthly allocation + apply, `BudgetStrategyInsightPanel` for target vs line mix), `src/data/repositories/budget-lines.ts`, overrides, `src/domain/finance/budget.ts` |
...
- **Canonical loan register (`20260628000000_financial_loans_register.sql`):** additive **`financial_loans`** table with source key / source row id, ownership flags, category, repayment metadata, funding source (`cash` \| `cpf_oa` \| `split`), CPF OA / cash portions, and source snapshots. Backfills existing generic liabilities, housing loans, and vehicle loans, then keeps the register synced by DB triggers on `financial_liabilities`, `financial_housing_loans`, and `financial_vehicles`. Old source tables and columns remain intact; this table is the parity/migration surface before any future source-table cleanup.
...
- **Shipped:** Monthly CPF path (`buildCpfMonthlyProjectionSeries`) forms RA at age 55 (SA then OA up to projected FRS) so Home **Projected CPF** and runway CPF breakdowns include RA; educational **`CpfRetirementProjectionPanel`** re-simulates from pre–set-aside OA/SA with BRS/FRS/ERS and CPF LIFE band, CPF Investments maturity routing after SA closure (RA top-up first, excess to OA), scenarios, collapsible assumptions.
