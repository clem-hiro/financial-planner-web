...
| **CPF retirement projection + CPF Investments** (OA/SA investment entries, FRS/BRS/ERS estimates, age-55 RA simulation, educational scenarios) | **Shipped** | Setup/Wealth → CPF & CPF Investments supports multiple OA/SA entries with purchase month, single vs regular premium, amount, growth, and maturity month. `buildCpfMonthlyProjectionSeries` deducts future CPF investment premiums, carries notional CPF investment value, and routes maturity proceeds: OA → OA; SA before 55 → SA; SA after SA closure → RA first, then OA. |
...
_Last reviewed (2026-05-25): CPF Investments MVP shipped with multiple OA/SA entries, regular/single premiums, maturity flow-back, and age-55 SA closure routing; `cpf-monthly-projection.ts`, `CpfBalancesForm.tsx`, migration `20260625000000`. Prior: CPFIS-SA maturity helper; `cpf-retirement-projection.ts`._
