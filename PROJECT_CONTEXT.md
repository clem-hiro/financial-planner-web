...
| **Advisor cash + liability + vehicle propose-CRUD** (Phases 2–4) | **Shipped** | Advisor compose can **create / update / delete cash accounts, liabilities, and vehicles** as staged proposals across all 5 seams: compose actions (`src/server/advisor-client-actions.ts`) with Zod allowlist + `entityNameUniquenessError` + the per-category gate; shared overlay branch (`apply-overlay.ts`); accept writer (`apply-changes.ts`, via the generic `applyEntityDiff`) with per-type name-conflict guard (name field is `name` for cash/liability, **`label`** for vehicle); and a shared **locked "Private" card** (`LockedCategoryCard`) in `AdvisorClientCompose` when the client hasn't shared the category. **Vehicle** exposes the **core 7** fields (label, status, market value, OTR, loan balance/payment/months); the full SG valuation set (PARF/COE/depreciation/anchors) stays in the client's `VehiclesPanel` — advisor create defaults those safely, edit preserves them. Liabilities + vehicles lack `updated_at` ⇒ those change rows carry **no `base_version`** (no optimistic-concurrency guard — last-write-wins on accept; delete-races safe; vehicle clobber surface limited to the core 7). Property / housing-loan (Phases 5–6) **not yet built** — accept rejects those `entity_type`s. _(Migrations `20260624`/`20260625` on prod, verified 2026-05-24.)_ |
