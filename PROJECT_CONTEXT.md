...
_Last reviewed (2026-05-25): investment withdrawal start age UX now converts age (for example 65) into the stored years-from-today offset before projection; CPF balance-as-of month + Dec year-end CPF snapshot shipped; 2026 CPF allocation ratios, post-55 RA contribution routing, and month-after-birthday age-band handling updated in `cpf-monthly-projection.ts` / `sg-cpf-contribution-buckets.ts`; migration `20260626000000`. Prior: CPF Investments MVP (`20260625000000`), CPFIS-SA maturity helper; `cpf-retirement-projection.ts`._
