...
| Profile income / CPF assumptions | **Shipped** | Setup **profile** tab — income, bonus, optional **other monthly income** (one take-home lump for side hustle/freelance), birth date, CPF salary path. Values seeded from onboarding appear as normal editable fields (no permanent sync banner). **Lifestyle Profile** preferences (lifestyle, typical food spend, money management style) live here as long-lived budgeting assumptions — no recommended allocation numbers. No retirement fields. Home **Income** = salary take-home + other monthly income (rental on Housing stays separate). |
...
| SG-oriented guided budget templates (onboarding + domain) | **Shipped** | Modular generator in `budget-guided-setup.ts` (`BudgetRecommendationContext` + active/future signal metadata); seeded in onboarding and applied from Setup → Budget. Future signals (housing, vehicles, goals, investments, cash flow, debt, CPF) reserved without hardcoding the engine to lifestyle alone. |
...
| Lifestyle Profile (Setup → Profile) | `src/features/setup/BudgetLensProfileForm.tsx` — lifestyle, food spend, money style preferences only (PATCH `/api/profile`); no allocation preview |
