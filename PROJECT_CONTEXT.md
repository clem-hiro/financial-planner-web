...
| SG-oriented guided budget templates (onboarding + domain) | **Shipped** | Modular generator in `budget-guided-setup.ts` (`BudgetRecommendationContext` + active/future signal metadata); seeded in onboarding and applied from Setup → Budget. **Apply Recommendation** creates lines only when the monthly budget is empty; if replaceable lines already exist, preview stays visible and replace requires an explicit confirm checkbox (debt/tax preserved). Future signals (housing, vehicles, goals, investments, cash flow, debt, CPF) reserved without hardcoding the engine to lifestyle alone. |
