...
- **Migration filenames unique per timestamp prefix.** Latest on disk: `20260529000000_advisor_consent_phase2.sql`. **Consent-gate migrations reserved from `20260528000000`.** Reconciled 2026-05-18 (CleAyz heads-up): the `20260525/26000000_profile_expense_growth_nominal` duplicate (cash-flow co-dev merge provenance; `26` was byte-identical/dead) — deleted `26`, renamed `25`→`20260525000001` (`20260524000000` was taken by `advisor_proposals`). Also reconciled the two remaining DISTINCT-migration dup-prefix pairs (rename one of each, never delete): the `20260427000000`×2 pair (`vehicle_loan_simple_remaining` + `vehicles_terminal_recovery_coe`) — bumped `vehicle_loan_simple_remaining` to prefix `20260427000001`, sibling kept at `20260427000000`; the `20260512000002`×2 pair — bumped the **unreferenced** `budget_onboarding_profile_extensions` to prefix `20260512000003`, leaving consent-adjacent `advisor_select_linked_clients` (whose `financial_profiles_select_advisor_clients` policy `20260529000000` drops) at `20260512000002` untouched with its 3 refs intact. Prefix-uniqueness now holds on disk. Filename hygiene only, no prod change (column already applied; migrations idempotent). _(added: 2026-05-14, updated: 2026-05-18)_
