...
argument-hint: "<migration-file>  | --latest | --all"
---
...
```bash
grep -nE '^\s*create table (public\.)?[a-z_]+' "$MIGRATION" \
...
| # | Invariant | Detection grep |
|---|---|---|
| 1 | RLS enabled | `grep -E "alter table (public\.)?$T enable row level security" $MIGRATION` |
| 2 | SELECT policy exists | `grep -E "create policy \".*\" on (public\.)?$T\\s+for select" $MIGRATION` |
...
```bash
grep -rEn "create policy \".*\" on (public\.)?$T\\s+for (select|insert|update|delete)" supabase/migrations/
```
