...
description: Pre-implementation gate that recommends the right test layer for a planned change. Maps the target file to the test layer per handbook §3.10 (pure domain → vitest unit; RLS/conflict-guard/middleware → integration; UI flow → manual scenario matrix). Surfaces the four currently-untested invariants from §3.10 so you know what's NOT covered.
...
| UI component | `src/features/**/*.tsx`, `src/ui/**/*.tsx` | Manual scenario matrix per `~/.claude/skills/testing-philosophy`. Skip unit tests on render-only components. |
...
1. Use the function tree to find inbound `*.test.ts` callers:
   ...
   ```
2. Also grep test files directly (the tree may miss imports the type-checker resolved to types-only):
   ```bash
   grep -rln "$SYMBOL" src/**/*.test.ts
   ```
...
Layer: server-action
Existing tests: none found via function tree or grep.
