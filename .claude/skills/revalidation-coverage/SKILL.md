...
description: Given a server action or route handler that performs a write, list every page route that reads the same table (via the function-tree) and verify each is covered by a `revalidatePath(...)` call. Catches stale-UI bugs where a new page consumes an existing repository but the mutation forgets to invalidate it. Reuses `docs/function-tree/function-tree.json`.
allowed-tools: Read, Bash, Grep
argument-hint: "<action-or-route-symbol-or-id>"
---
...
Parse `$ARGUMENTS` to a function ID (use the same resolution as `/blast-radius`: bare symbol → unique ID via `jq` over modules[].functions[]).
...
- The function tree v1 does NOT track value-passing edges (`<form action={myAction}>`). Coverage will undercount when actions are bound by JSX prop. Cross-check with `grep -rn "myAction" src/features/`.
